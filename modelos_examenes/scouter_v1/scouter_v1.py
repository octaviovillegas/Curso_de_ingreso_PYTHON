...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.image = tk.PhotoImage(file='./modelos_examenes/scouter/UTN_Scouter_App_v1.png')
        self.top_banner = customtkinter.CTkLabel(master = self, image = self.image, text = '')
        self.top_banner.grid_configure(row = 1, column = 0, padx = 20, pady = 5, columnspan = 2, rowspan = 1, sticky = 'we')

        self.btn_cargar = customtkinter.CTkButton(master=self, text="Cargar Participantes", command=self.btn_cargar_pokedex_on_click)
